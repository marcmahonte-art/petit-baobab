  ...
  return (
    <div className="rounded-[28px] bg-white p-5 shadow-[0_4px_12px_rgba(0,0,0,.06)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {colorings.map((c, i) => (
          <div key={i} className="coloring-item h-[280px] rounded-[18px] border border-[#ECECEC] p-3 flex flex-col bg-white">
            <div className="h-[180px] rounded-[12px] bg-[#FFF9F2] flex items-center justify-center overflow-hidden relative">
              <Image
                src={c.image}
                alt={c.title}
                width={120}
              ...
              />
              <div className="absolute bottom-2 right-2 w-8 h-8 rounded-full bg-[#FFE08A] flex items-center justify-center">
                <Pencil className="w-4 h-4 text-[#3B2416]" />
              </div>
            </div>
            <div className="flex-1 flex flex-col justify-center pt-2">
              <span className="text-sm font-bold text-[#3B2416] leading-tight">{c.title}</span>
              <span className="text-xs text-[#7A6A5E] mt-0.5">{c.time}</span>
            </div>
          </div>
        ))}
      </div>
