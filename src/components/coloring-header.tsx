      ...
      <div className="flex flex-row items-center gap-[12px] md:gap-[24px] w-full md:w-auto justify-between md:justify-end">
        ...
        <div className="relative">
          <div
            ...
            className="flex items-center gap-1.5 md:gap-2 h-[44px] md:h-[52px] rounded-full border border-[#EFE7DB] pl-1 pr-2.5 md:pl-1.5 md:pr-3.5 bg-white cursor-pointer hover:bg-neutral-50 transition-colors shadow-sm"
          >
            <Avatar className="w-7 h-7 md:w-9 md:h-9 border border-neutral-100">
              <AvatarImage src={getAvatarSrc(profileMascot)} />
              <AvatarFallback>{profileName.substring(0, 2).toUpperCase()}</AvatarFallback>
            </Avatar>
            <div className="flex items-center gap-1">
              <span className="text-[12px] md:text-[14px] font-extrabold text-[#3B2416] capitalize">{profileName}</span>
              <ChevronDown className="w-3 h-3 md:w-3.5 md:h-3.5 text-[#7A6A5E]" />
            </div>
          </div>
