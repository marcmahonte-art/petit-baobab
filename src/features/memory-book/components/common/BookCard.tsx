...
import { BookOpen, Calendar, ChevronRight, Trash2, Camera, Printer } from "lucide-react";
...
export const BookCard: React.FC<BookCardProps> = ({ book, onDelete }) => {
  const legacyPages = book.pages_data as LegacyPagesData;
  const answers: Record<string, string> =
    book.answers_data ||
    (Array.isArray(book.pages_data) && legacyPages.answers) ||
    {};
  ...
  return (
    <div className="group bg-[#FFF9F2] rounded-[24px] border border-[#F0E7DA] hover:border-[#7D6AF8] shadow-[0_2px_10px_rgba(59,36,22,0.06)] hover:shadow-[0_10px_30px_-14px_rgba(59,36,22,0.18)] transition-all p-5 flex flex-col justify-between relative overflow-hidden font-['Quicksand',sans-serif]">
      {/* Spirale décorative sur le bord gauche */}
      <div className="absolute top-0 bottom-0 left-0 w-4 bg-[#F3EDE4] border-r border-dashed border-[#C9BFA9] flex flex-col justify-evenly py-2 items-center">
      ...
      <div className="pl-3">
        ...
        {/* Détails du contenu */}
        <div className="bg-white rounded-2xl border border-[#F0E7DA] p-2.5 mb-4 flex items-center justify-between text-xs font-bold text-[#5B5648]">
          <span className="flex items-center gap-1.5">
            <BookOpen className="w-3.5 h-3.5 text-[#7D6AF8]" />
            10 pages complètes
          </span>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-[#C9BFA9]/60">
          <Link
            ...
            className="flex-1 py-2 px-3 rounded-full bg-[#7D6AF8] hover:bg-[#6d59ef] text-white font-bold text-xs flex items-center justify-center gap-1 shadow-xs transition active:scale-98"
          >
          ...
          <Link
            ...
            className="p-2 rounded-full bg-white hover:bg-[#F3EBDA] border border-[#F0E7DA] text-[#3A362E] font-bold text-xs transition flex items-center gap-1"
